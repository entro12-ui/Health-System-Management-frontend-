...
const EyeDoctor = () => {
  ...
  useEffect(() => {
    const stored = localStorage.getItem('viewedCardNumbers_eye');
    if (stored) {
  ...
  const fetchAssigned = async () => {
    try {
      const response = await fetch('http://localhost:4000/doctor-notifications/type?type=eye');
  ...
  const handleAccept = (patient) => {
    ...
    localStorage.setItem('viewedCardNumbers_eye', JSON.stringify(updated));
  ...
  return (
    <Card title="Eye Doctor Notifications" style={{ margin: '40px auto', maxWidth: 1000 }}>
